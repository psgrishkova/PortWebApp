...
public class ShipRepository {
    ...
    public static List<Ship> findShipByName(String name) {
        ...
        try {
            PreparedStatement ps = DBConnection.getPreparedStatement("Select * from ship where name like ?");
            ps.setString(1,name);
    ...
    public static List<Ship> findShipByNum(String num) {
        List<Ship> ships = new LinkedList<>();
        try {
            PreparedStatement ps = DBConnection.getPreparedStatement("Select * from ship where reg_number like ?");
            ps.setString(1,num);
            ResultSet rs = ps.executeQuery();
            while (rs.next())
                ships.add(new Ship(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getInt(4)));
